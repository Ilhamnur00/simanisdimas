    ...
    <div class="max-w-7xl mx-auto py-10 px-6">
        ...
        <div class="mb-8">
            ...
            <p class="text-slate-500 mt-1 text-base">
                Laporan perawatan untuk perangkat:
                <span class="text-teal-600 font-semibold">{{ $device->nama }}</span>
            </p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-slate-100">
                    @forelse ($riwayat as $index => $item)
                        <tr class="hover:bg-sky-50 transition duration-150">
                            ...
                            <td class="px-6 py-4">
                                @if ($item->bukti)
                                    <a href="{{ asset('storage/' . $item->bukti) }}" target="_blank" class="text-sky-600 hover:underline">
                                        Lihat
                                    </a>
                                @else
                        ...
                        <div id="modal-{{ $index }}" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 hidden">
                            <div class="bg-white rounded-xl shadow-2xl w-full max-w-2xl p-6 relative animate-fade-in-down overflow-y-auto max-h-screen">
                                ...
                                <div class="space-y-3 text-sm text-slate-700">
                                    ...
                                    <p><strong>Kategori:</strong> {{ $item->status }}</p>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center py-6 text-slate-400 italic">Belum ada laporan perawatan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
