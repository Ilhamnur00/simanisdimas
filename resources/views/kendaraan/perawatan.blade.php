<x-app-layout>
    ...
    <div class="max-w-5xl mx-auto py-12 px-6">
        ...
        @if(isset($kendaraan))
            ...
            <div class="bg-gradient-to-br from-sky-50 to-white border border-sky-100 shadow-md rounded-2xl p-6 mb-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-3 text-sm text-slate-800">
                    ...
                    <p><span class="font-medium">No Polisi:</span> {{ $kendaraan->no_polisi ?? '-' }}</p>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-8 shadow-xl">
                <h4 class="text-2xl font-semibold text-sky-800 mb-6">Formulir Laporan Perawatan</h4>

                <form method="POST" action="{{ route('perawatan.store', $kendaraan->id) }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <!-- Tanggal -->
                        <div>
                            <label for="tanggal" class="block text-sm font-medium text-slate-700 mb-1">Tanggal Perawatan</label>
                        ...
                        <!-- Kategori -->
                        <div>
                            <label for="kategori_perawatan" class="block text-sm font-medium text-slate-700 mb-1">Kategori Perawatan</label>
                            <select name="kategori_perawatan" id="kategori_perawatan" required
                                ...
                                <option value="Penggantian Aki">Penggantian Aki</option>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="4" required
                                class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-sky-500 focus:ring focus:ring-sky-100 transition"
                                placeholder="Jelaskan kondisi kendaraan atau tindakan perawatan yang dilakukan..."></textarea>
                        </div>
                    </div>
        ...
        @else
            <div class="bg-red-50 border border-red-200 text-red-700 p-6 rounded-lg shadow text-center">
                <p><strong>Error:</strong> Data kendaraan tidak tersedia.</p>
                <a href="{{ route('kendaraan.index') }}" class="text-blue-600 underline mt-3 inline-block">Kembali ke daftar kendaraan</a>
            </div>
        @endif
    </div>
